...
const CLSComponent = () => {
  ...
  const highlightCLSRect = (previousRect, newRect) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }).then(([tab]) => {
      chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: (previousRect, newRect) =>
          window._highlightCLSShiftRects(previousRect, newRect),
  ...
  return (
    <div className={styles.inpContainer}>
      <VitalHeadingContainer
        ...
        description={DESCRIPTION_TEXT.CLS}
      />
      <VitalMainIndicator
        good={0.1}
        poor={0.25}
      ...
      {webVitalsData['CLS'].value >= 0 ? (
        <>
          ...
          <p className={styles.sectionDescription}>
            Click on the entry to visualize the layout shift in page.
          </p>
          <div className={styles.clsTopicContainer}>
            <div className={`${styles.clsNodeSelectorText} bold-text`}>
              Node Selector
            </div>
            <div className={`${styles.clsStartTimeText} bold-text`}>
              Start Time
            </div>
          ...
          <div className={styles.clsTopContainer}>
            {clsTopEntriesData.map((entry) => {
              return (
                <div
                  className={styles.clsRowContainer}
                  onClick={() => highlightCLSRect(entry.oldRect, entry.newRect)}
                >
                  <div className={styles.clsNodeSelectorText}>
                    {entry.nodeSelector}
                  </div>
                  <div className={styles.clsStartTimeText}>
                    {entry.startTime.toFixed(3)}
                  </div>
                  <div className={styles.clsValueText}>
