...
const LCPContainer = () => {
  ...
  useEffect(() => {
    if(webVitalsData["LCP"].value >= 0) {
      ...
      setLCPDebugInfo((debugInfo) => ({...debugInfo,navigationType: webVitalsData["LCP"].navigationType}));
  ...
  return (
    <div className={styles.lcpContainer}>
      <VitalHeadingContainer vitalType="LCP" value={webVitalsData["LCP"].value} description={DESCRIPTION_TEXT.LCP} />
        <VitalMainIndicator good={2.5} poor={4} unit="s" value={webVitalsData["LCP"].value} variant="LCP"/>
        { webVitalsData["LCP"].value >= 0 ?
          <>
          <p className={styles.sectionTopic}>Element Info:</p>
          <p className={styles.sectionDescription}>Click on the element to highlight the Largest Contentful Paint element in page.</p>
          <div className={styles.elementTypeContainer} onClick={highlightLCPElement}>
